...
import (
	...
	"github.com/TokensZhuanfa/dujiao-shop/internal/authz"
	"github.com/TokensZhuanfa/dujiao-shop/internal/cache"
	"github.com/TokensZhuanfa/dujiao-shop/internal/config"
	"github.com/TokensZhuanfa/dujiao-shop/internal/constants"
	adminhandlers "github.com/TokensZhuanfa/dujiao-shop/internal/http/handlers/admin"
	channelhandlers "github.com/TokensZhuanfa/dujiao-shop/internal/http/handlers/channel"
	publichandlers "github.com/TokensZhuanfa/dujiao-shop/internal/http/handlers/public"
	upstreamhandlers "github.com/TokensZhuanfa/dujiao-shop/internal/http/handlers/upstream"
	"github.com/TokensZhuanfa/dujiao-shop/internal/http/response"
	"github.com/TokensZhuanfa/dujiao-shop/internal/logger"
	"github.com/TokensZhuanfa/dujiao-shop/internal/provider"
	"github.com/TokensZhuanfa/dujiao-shop/internal/web"
...
func SetupRouter(cfg *config.Config, c *provider.Container) *gin.Engine {
	...
	// guestOrderRule 用于游客订单 GET 流量（详情 / 文件下载 / Codex 下载等）。
	// 攻击模型：脚本用 "email + 字典 + order_no 枚举" 拉别人的 Codex token 等敏感交付。
	// 策略：每个客户端 IP 每分钟最多 30 次，超出后封 5 分钟。正常用户每个订单刷新页面 + 下载几次远不到上限。
	guestOrderRule := RateLimitRule{
		Prefix:        fmt.Sprintf("%s:rate:guest_order", redisPrefix),
		WindowSeconds: 60,
		MaxRequests:   30,
		BlockSeconds:  300,
		MessageKey:    "error.rate_limited",
	...
	apiV1 := r.Group("/api/v1")
	{
		...
		guest := apiV1.Group("/guest")
		{
			...
			guest.GET("/orders", RateLimitMiddleware(redisClient, guestOrderRule, KeyByIP), publicHandler.ListGuestOrders)
			guest.GET("/orders/:order_no", RateLimitMiddleware(redisClient, guestOrderRule, KeyByIP), publicHandler.GetGuestOrderByOrderNo)
			guest.GET("/orders/:order_no/fulfillment/download", RateLimitMiddleware(redisClient, guestOrderRule, KeyByIP), publicHandler.DownloadGuestFulfillment)
			guest.GET("/orders/:order_no/fulfillment/files", RateLimitMiddleware(redisClient, guestOrderRule, KeyByIP), publicHandler.ListGuestFulfillmentFiles)
			guest.GET("/orders/:order_no/fulfillment/files/archive", RateLimitMiddleware(redisClient, guestOrderRule, KeyByIP), publicHandler.DownloadGuestFulfillmentArchive)
			guest.GET("/orders/:order_no/fulfillment/files/:id/download", RateLimitMiddleware(redisClient, guestOrderRule, KeyByIP), publicHandler.DownloadGuestFulfillmentFile)
			guest.GET("/orders/:order_no/codex-accounts", RateLimitMiddleware(redisClient, guestOrderRule, KeyByIP), publicHandler.ListGuestOrderCodexAccounts)
			guest.GET("/orders/:order_no/codex-accounts/:format/:id", RateLimitMiddleware(redisClient, guestOrderRule, KeyByIP), publicHandler.DownloadGuestOrderCodexAccount)
			guest.GET("/orders/:order_no/codex-accounts/:format/archive", RateLimitMiddleware(redisClient, guestOrderRule, KeyByIP), publicHandler.DownloadGuestOrderCodexAccountsArchive)
		...
		user.Use(UserJWTAuthMiddleware(cfg.UserJWT.SecretKey, c.UserRepo))
		{
			...
			user.GET("/orders/:order_no/fulfillment/files", publicHandler.ListFulfillmentFiles)
			user.GET("/orders/:order_no/fulfillment/files/archive", publicHandler.DownloadFulfillmentArchive)
			user.GET("/orders/:order_no/fulfillment/files/:id/download", publicHandler.DownloadFulfillmentFile)
			user.GET("/orders/:order_no/codex-accounts", publicHandler.ListOrderCodexAccounts)
			user.GET("/orders/:order_no/codex-accounts/:format/:id", publicHandler.DownloadOrderCodexAccount)
			user.GET("/orders/:order_no/codex-accounts/:format/archive", publicHandler.DownloadOrderCodexAccountsArchive)
		...
		admin := apiV1.Group("/admin")
		{
			...
			authorized := admin.Use(JWTAuthMiddleware(cfg.JWT.SecretKey, c.AdminRepo), AdminRBACMiddleware(c.AuthzService))
			{
				...
				authorized.POST("/products/:id/shared-file", adminHandler.UploadProductSharedFile)
				authorized.DELETE("/products/:id/shared-file", adminHandler.DeleteProductSharedFile)
				...
				authorized.POST("/card-secrets/import-files", adminHandler.ImportCardSecretFiles)
				...
				// 号池 - Codex
				authorized.GET("/number-pool/codex", adminHandler.GetCodexAccounts)
				authorized.GET("/number-pool/codex/settings", adminHandler.GetCodexPoolSettings)
				authorized.PUT("/number-pool/codex/settings", adminHandler.SaveCodexPoolSettings)
				authorized.POST("/number-pool/codex/proxy/test", adminHandler.TestCodexProxy)
				authorized.POST("/number-pool/codex/import", adminHandler.ImportCodexAccounts)
				authorized.POST("/number-pool/codex/batch", adminHandler.BatchCodexAccount)
				authorized.PATCH("/number-pool/codex/:id", adminHandler.UpdateCodexAccount)
				authorized.PATCH("/number-pool/codex/:id/status", adminHandler.SetCodexAccountStatus)
				authorized.POST("/number-pool/codex/:id/refresh", adminHandler.RefreshCodexAccount)
				authorized.POST("/number-pool/codex/:id/usage", adminHandler.FetchCodexAccountUsage)
				authorized.DELETE("/number-pool/codex/:id", adminHandler.DeleteCodexAccount)
