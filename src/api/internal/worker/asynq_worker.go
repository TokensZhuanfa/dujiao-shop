...
import (
	...
	"github.com/TokensZhuanfa/dujiao-shop/internal/htmltext"
	...
	"github.com/TokensZhuanfa/dujiao-shop/internal/telegramidentity"
	"github.com/TokensZhuanfa/dujiao-shop/internal/upstream"
...
func (c *Consumer) handleOrderStatusEmail(_ context.Context, task *asynq.Task) error {
	...
	} else if payloadText == "" && orderHasFileAttachments(order) &&
		(status == constants.OrderStatusDelivered || status == constants.OrderStatusCompleted) {
		// 文件型卡密：邮件不附带文件本体（可达 1 GB），改为引导用户去订单详情页下载
		input.FulfillmentInfo = "您购买的商品包含可下载文件，请前往订单详情页查看附件并下载。\n" +
			"Your order includes downloadable files. Please open the order detail page to access them."
	} else {
...
func (c *Consumer) handleOrderAutoFulfill(_ context.Context, task *asynq.Task) error {
	...
	if err != nil {
		...
		case errors.Is(err, service.ErrCardSecretInsufficient):
			// 卡密 / 号池库存耗尽。重试也注定失败——已经付款的订单需要管理员介入退款 / 补货。
			// 不再丢回 asynq 死循环重试，留 warn 日志便于巡检（按 order_id 搜）。
			logger.Warnw("worker_order_auto_fulfill_insufficient_stock",
				"order_id", payload.OrderID,
				"error", err.Error(),
				"action_required", "manual_intervention: refund or replenish pool")
			return nil
		default:
...
// orderHasFileAttachments 判断订单是否包含文件型交付（per-stock 文件型卡密 或 共享文件型商品）。
// 用于邮件/Bot 通知时改换为"前往订单页下载"的提示文案，避免空交付内容。
func orderHasFileAttachments(order *models.Order) bool {
	if order == nil {
		return false
	}
	checkItems := func(items []models.OrderItem) bool {
		for _, it := range items {
			t := strings.TrimSpace(it.FulfillmentType)
			if t == constants.FulfillmentTypeAuto {
				// 仅靠 OrderItem 判定不出 AutoSecretKind；这里返回 true 即可触发提示文案，
				// 详细附件由订单详情页另行展示。文本型订单 payload 非空，不会进入本分支。
				return true
			}
		}
		return false
	}
	if checkItems(order.Items) {
		return true
	}
	for _, child := range order.Children {
		if checkItems(child.Items) {
			return true
		}
	}
	return false
