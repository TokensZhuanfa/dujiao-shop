...
type Product struct {
	...
	AutoSecretKind       string         `gorm:"type:varchar(16);not null;default:'text'" json:"auto_secret_kind"`   // 自动发货卡密类型（text/file_per_stock/file_shared）
	SharedFilePath       string         `gorm:"type:varchar(512)" json:"-"`                                          // 共享文件型卡密的私有存储相对路径
	SharedFileOriginalName string       `gorm:"type:varchar(255)" json:"shared_file_original_name,omitempty"`        // 共享文件原始文件名
	SharedFileSize       int64          `gorm:"default:0" json:"shared_file_size,omitempty"`                         // 共享文件大小（字节）
	SharedFileMimeType   string         `gorm:"type:varchar(127)" json:"shared_file_mime_type,omitempty"`            // 共享文件 MIME 类型
