...
const (
	...
	SettingKeyCodexPoolConfig          = "codex_pool_config"
...
const (
	...
	CardSecretSourceFiles  = "files"
)

// 卡密内容类型常量（区分文本卡密与文件卡密）
const (
	CardSecretKindText = "text"
	CardSecretKindFile = "file"
)

// 自动发货卡密种类常量（决定 Product 自动交付模式）
const (
	AutoSecretKindText         = "text"           // 文本卡密池（默认）
	AutoSecretKindFilePerStock = "file_per_stock" // 每个文件占一份库存，按购买数量发放
	AutoSecretKindFileShared   = "file_shared"    // 单个共享文件，无限库存
	AutoSecretKindCodexPool    = "codex_pool"     // 从 Codex 号池中按 sold=false & status=ok 自动挑账号交付
)

// 自动发货库存约定：-1 表示无限（沿用 ManualStockUnlimited 既有约定）
const (
	AutoStockUnlimited = -1
)
