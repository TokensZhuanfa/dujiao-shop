...
func NewCardSecretService(secretRepo repository.CardSecretRepository, batchRepo repository.CardSecretBatchRepository, productRepo repository.ProductRepository, productSKURepo repository.ProductSKURepository, credentialFileService *CredentialFileService) *CardSecretService {
...
// ImportCardSecretFilesInput 文件型卡密导入输入
type ImportCardSecretFilesInput struct {
	ProductID uint
	SKUID     uint
...
// ImportCardSecretFiles 把若干上传文件作为文件型卡密入库。每个文件 = 一份独立库存。
// 同时会把 Product.AutoSecretKind 切换为 file_per_stock，让交付走文件路径。
func (s *CardSecretService) ImportCardSecretFiles(input ImportCardSecretFilesInput) (*models.CardSecretBatch, int, error) {
	if input.ProductID == 0 {
		return nil, 0, ErrCardSecretInvalid
	}
	...
	// 先把所有文件保存到私有目录；任何一个失败都要回滚已落盘的文件
	saved := make([]*CredentialFileResult, 0, len(input.Files))
	cleanup := func() {
		for _, r := range saved {
			_ = s.credentialFileService.Delete(r.RelPath)
		}
	}
	for _, fh := range input.Files {
		...
		res, err := s.credentialFileService.SaveCredentialFile(fh)
		if err != nil {
			cleanup()
			return nil, 0, ErrCardSecretImportFailed
		}
	...
	batch := &models.CardSecretBatch{
		...
		SKUID:      sku.ID,
		BatchNo:    batchNo,
		Source:     constants.CardSecretSourceFiles,
		TotalCount: len(saved),
		Note:       strings.TrimSpace(input.Note),
		CreatedAt:  now,
		UpdatedAt:  now,
	}
	...
	err = s.secretRepo.Transaction(func(tx *gorm.DB) error {
		...
		items := make([]models.CardSecret, 0, len(saved))
		for _, r := range saved {
			items = append(items, models.CardSecret{
				ProductID:        input.ProductID,
				SKUID:            sku.ID,
				BatchID:          &batch.ID,
				Secret:           "", // 文件型卡密 Secret 为空串
				Kind:             constants.CardSecretKindFile,
				FilePath:         r.RelPath,
		...
		// 强制商品的自动发货模式为文件型，避免文本/文件混用导致交付时 payload 行为不一致
		if err := tx.Model(&models.Product{}).Where("id = ?", input.ProductID).
			Update("auto_secret_kind", constants.AutoSecretKindFilePerStock).Error; err != nil {
			return ErrCardSecretBatchCreateFailed
		}
		return nil
	})
