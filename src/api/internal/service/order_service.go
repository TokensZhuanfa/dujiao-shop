...
func NewOrderService(opts OrderServiceOptions) *OrderService {
	return &OrderService{
		...
		codexAccountRepo:      opts.CodexAccountRepo,
...
func (s *OrderService) CreateGuestOrder(input CreateGuestOrderInput) (*models.Order, error) {
	...
	// 防爆破：游客密码最少 8 位、最多 64 位。
	// 长度过短会让攻击者凭"已知 email + 字典"在限流之内枚举出 Codex token 等敏感交付。
	if len(password) < 8 {
		return nil, ErrGuestPasswordRequired
	}
	if len(password) > 64 {
	...
	err = s.orderRepo.Transaction(func(tx *gorm.DB) error {
		...
		for idx := range result.Plans {
			...
			if strings.TrimSpace(plan.Item.FulfillmentType) == constants.FulfillmentTypeAuto {
				// file_shared 模式：单文件共享，无库存概念，跳过卡密预占
				if plan.Product != nil && strings.TrimSpace(plan.Product.AutoSecretKind) == constants.AutoSecretKindFileShared {
					// noop — 在 fulfillment 阶段从 Product.SharedFile* 直接读取
				} else if plan.Product != nil && strings.TrimSpace(plan.Product.AutoSecretKind) == constants.AutoSecretKindCodexPool {
					// codex_pool：在下单事务里直接预占号池账号，对齐文本卡密 Reserve 语义。
					// 库存不足 → ErrCardSecretInsufficient 让外层事务回滚，订单不会创建出来。
					if s.codexAccountRepo == nil {
						return ErrCardSecretInsufficient
					}
					affected, err := s.codexAccountRepo.WithTx(tx).ReserveForOrder(tx, childOrder.ID, plan.Item.Quantity, now)
					if err != nil {
						return err
					}
					if int(affected) != plan.Item.Quantity {
						return ErrCardSecretInsufficient
					}
				} else {
