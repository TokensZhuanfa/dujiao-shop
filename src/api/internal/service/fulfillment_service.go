...
import (
	...
	"github.com/TokensZhuanfa/dujiao-shop/internal/queue"
...
func (s *FulfillmentService) CreateAuto(orderID uint) (*models.Fulfillment, error) {
	...
	err = s.orderRepo.Transaction(func(tx *gorm.DB) error {
		...
		// 预加载本订单涉及的所有商品，用于读取 AutoSecretKind 决定走文本/文件分支
		productKinds := make(map[uint]string)
		if s.productRepo != nil {
			...
			for _, pid := range productIDs {
				p, err := s.productRepo.WithTx(tx).GetByID(strconv.FormatUint(uint64(pid), 10))
				if err != nil {
					return err
				}
				if p == nil {
				...
				kind := strings.TrimSpace(p.AutoSecretKind)
				if kind == "" {
					kind = constants.AutoSecretKindText
				}
				productKinds[pid] = kind
			}
		}
		...
		var codexPayloadLines []string // codex_pool 模式的 payload 行
		for _, item := range order.Items {
			if item.ProductID == 0 || item.Quantity <= 0 {
				return ErrFulfillmentInvalid
			}
			// file_shared 模式不消耗卡密池：所有买家共享同一个文件，库存视为无限
			if productKinds[item.ProductID] == constants.AutoSecretKindFileShared {
				continue
			}
			// codex_pool 模式：从已预占的账号里挑出来转 sold（预占在下单事务里已完成）
			if productKinds[item.ProductID] == constants.AutoSecretKindCodexPool {
				if s.codexAccountRepo == nil {
					return ErrFulfillmentCreateFailed
				}
				accounts, err := s.codexAccountRepo.WithTx(tx).AllocateForOrder(tx, orderID, order.OrderNo, item.Quantity, now)
				if err != nil {
					if err.Error() == "codex_pool_insufficient" || err.Error() == "codex_pool_race" {
						return ErrCardSecretInsufficient
					}
					return err
				}
				for _, a := range accounts {
					line, err := buildCodexAuthPayload(&a)
					if err != nil {
						return err
					}
					codexPayloadLines = append(codexPayloadLines, line)
				}
				continue
			}
		...
		for _, secret := range secrets {
			...
			// 文件型卡密的 Secret 字段为空串，不参与文本 payload 拼接
			if strings.TrimSpace(secret.Kind) == constants.CardSecretKindFile {
				continue
			}
		...
		payload := strings.Join(append(secretLines, codexPayloadLines...), "\n")
		fulfillment = &models.Fulfillment{
			OrderID:     orderID,
...
// buildCodexAuthPayload 把一个 codex 账号序列化成可直接喂给 codex-assistant 的 auth.json 文本。
// 字段结构与现有 ParseAnyAuth 兼容（tokens.{access_token,id_token,refresh_token,account_id}），
// 顶层再额外带 email / plan / account_id 便于买家肉眼识别。
func buildCodexAuthPayload(acc *models.CodexAccount) (string, error) {
	if acc == nil {
		return "", errors.New("nil codex account")
	}
