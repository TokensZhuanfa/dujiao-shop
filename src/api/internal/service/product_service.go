...
type ProductService struct {
	...
	credentialFileService *CredentialFileService
}
...
func (s *ProductService) Create(input CreateProductInput) (*models.Product, error) {
	...
	product := models.Product{
		...
		AutoSecretKind:       normalizeAutoSecretKind(input.AutoSecretKind, fulfillmentType),
...
func (s *ProductService) Update(id string, input CreateProductInput) (*models.Product, error) {
	...
	// 自动交付下允许管理员把卡密种类切换为 text / codex_pool；
	// file_per_stock / file_shared 仍由专用接口写入（保持现有行为，避免误改丢库存）。
	// 切走 codex_pool 前若 product 当前是 codex_pool，新值必须显式指定。
	if fulfillmentType == constants.FulfillmentTypeAuto {
		newKind := normalizeAutoSecretKind(input.AutoSecretKind, fulfillmentType)
		curr := strings.TrimSpace(product.AutoSecretKind)
		// 不允许通过此接口覆盖文件型（避免误删上传的库存）；只允许 text <-> codex_pool 互切
		if curr == constants.AutoSecretKindFilePerStock || curr == constants.AutoSecretKindFileShared {
			// 保留原值
		} else {
			product.AutoSecretKind = newKind
		}
	} else {
		product.AutoSecretKind = constants.AutoSecretKindText
	}
	if fulfillmentType == constants.FulfillmentTypeManual {
		_, normalizedSchemaJSON, err := parseManualFormSchema(models.JSON(input.ManualFormSchemaJSON))
		if err != nil {
...
// normalizeAutoSecretKind 规范化交付方式下的卡密种类。
//   - fulfillment_type != auto → 强制 text
//   - file_per_stock / file_shared 通常由专用接口（导入文件、上传共享文件）切换，
//     管理员表单一般只设置 text 或 codex_pool；其它合法值也透传
func normalizeAutoSecretKind(raw, fulfillmentType string) string {
	if fulfillmentType != constants.FulfillmentTypeAuto {
		return constants.AutoSecretKindText
	}
	value := strings.ToLower(strings.TrimSpace(raw))
	switch value {
	case "", constants.AutoSecretKindText:
		return constants.AutoSecretKindText
	case constants.AutoSecretKindFilePerStock:
		return constants.AutoSecretKindFilePerStock
	case constants.AutoSecretKindFileShared:
		return constants.AutoSecretKindFileShared
	case constants.AutoSecretKindCodexPool:
		return constants.AutoSecretKindCodexPool
	default:
		return constants.AutoSecretKindText
	}
}
...
func (s *ProductService) ApplyAutoStockCounts(products []models.Product) error {
	...
	for i := range products {
		...
		// file_shared 模式视为无限库存，沿用 -1 约定，不参与卡密池计数
		if strings.TrimSpace(products[i].AutoSecretKind) == constants.AutoSecretKindFileShared {
			products[i].AutoStockAvailable = constants.AutoStockUnlimited
			products[i].AutoStockTotal = constants.AutoStockUnlimited
			products[i].AutoStockLocked = 0
			products[i].AutoStockSold = 0
			for j := range products[i].SKUs {
				products[i].SKUs[j].AutoStockAvailable = constants.AutoStockUnlimited
				products[i].SKUs[j].AutoStockTotal = constants.AutoStockUnlimited
				products[i].SKUs[j].AutoStockLocked = 0
				products[i].SKUs[j].AutoStockSold = 0
			}
			continue
		}
		// codex_pool 模式：库存 = 未售且 status=ok 的号池账号数。
		// 不和 card_secrets 卡密池共享库存计数；锁定量按 0 处理（分配在交付事务内一次性完成，没有半锁状态）。
		if strings.TrimSpace(products[i].AutoSecretKind) == constants.AutoSecretKindCodexPool {
			avail := int64(0)
			if s.codexAccountRepo != nil {
				if v, err := s.codexAccountRepo.CountAvailableForPool(); err == nil {
					avail = v
				}
			}
			products[i].AutoStockAvailable = avail
			products[i].AutoStockTotal = avail
			products[i].AutoStockLocked = 0
			products[i].AutoStockSold = 0
			for j := range products[i].SKUs {
				products[i].SKUs[j].AutoStockAvailable = avail
				products[i].SKUs[j].AutoStockTotal = avail
				products[i].SKUs[j].AutoStockLocked = 0
				products[i].SKUs[j].AutoStockSold = 0
			}
			continue
...
// SetSharedFile 给商品上传 / 替换共享文件型卡密。会把 AutoSecretKind 切换为 file_shared。
// 旧文件会在新文件落盘成功之后再删除（保证替换操作的原子性）。
func (s *ProductService) SetSharedFile(productID uint, file *multipart.FileHeader) (*models.Product, error) {
	if productID == 0 || file == nil {
		return nil, ErrCardSecretInvalid
	}
	if s.credentialFileService == nil {
		return nil, ErrCardSecretBatchCreateFailed
	}
	product, err := s.repo.GetByID(strings.TrimSpace(strconv.FormatUint(uint64(productID), 10)))
	if err != nil {
	...
	res, err := s.credentialFileService.SaveCredentialFile(file)
	if err != nil {
		return nil, ErrCardSecretImportFailed
	}
	oldPath := strings.TrimSpace(product.SharedFilePath)
	now := time.Now()
	updates := map[string]interface{}{
		"auto_secret_kind":          constants.AutoSecretKindFileShared,
		"shared_file_path":          res.RelPath,
		"shared_file_original_name": res.OriginalFilename,
		"shared_file_size":          res.Size,
		"shared_file_mime_type":     res.ContentType,
		"updated_at":                now,
	}
	if err := s.repo.QuickUpdate(strconv.FormatUint(uint64(productID), 10), updates); err != nil {
		_ = s.credentialFileService.Delete(res.RelPath)
		return nil, ErrProductUpdateFailed
	}
	if oldPath != "" && oldPath != res.RelPath {
		_ = s.credentialFileService.Delete(oldPath)
	}
	updated, err := s.repo.GetByID(strings.TrimSpace(strconv.FormatUint(uint64(productID), 10)))
	if err != nil {
		return nil, ErrProductFetchFailed
	}
	return updated, nil
...
// ClearSharedFile 删除商品的共享文件型卡密，AutoSecretKind 回退到 text。
func (s *ProductService) ClearSharedFile(productID uint) (*models.Product, error) {
	if productID == 0 {
		return nil, ErrCardSecretInvalid
	}
	product, err := s.repo.GetByID(strings.TrimSpace(strconv.FormatUint(uint64(productID), 10)))
	...
	now := time.Now()
	updates := map[string]interface{}{
		"auto_secret_kind":          constants.AutoSecretKindText,
		"shared_file_path":          "",
		"shared_file_original_name": "",
		"shared_file_size":          0,
		"shared_file_mime_type":     "",
		"updated_at":                now,
	}
	if err := s.repo.QuickUpdate(strconv.FormatUint(uint64(productID), 10), updates); err != nil {
		return nil, ErrProductUpdateFailed
	}
	if oldPath != "" && s.credentialFileService != nil {
		_ = s.credentialFileService.Delete(oldPath)
	}
	updated, err := s.repo.GetByID(strings.TrimSpace(strconv.FormatUint(uint64(productID), 10)))
	if err != nil {
