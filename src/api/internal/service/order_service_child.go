...
func (s *OrderService) cancelOrderWithChildren(order *models.Order, rollbackCoupon bool) error {
	...
	err := s.orderRepo.Transaction(func(tx *gorm.DB) error {
		...
		// codex_pool 预占同样需要释放（取消订单 / 超时未付都会走到这里）
		if s.codexAccountRepo != nil {
			codexRepo := s.codexAccountRepo.WithTx(tx)
			if len(order.Children) > 0 {
				for _, child := range order.Children {
					if _, err := codexRepo.ReleaseReservationByOrder(tx, child.ID); err != nil {
						return err
					}
				}
			} else {
				if _, err := codexRepo.ReleaseReservationByOrder(tx, order.ID); err != nil {
					return err
				}
			}
		}
