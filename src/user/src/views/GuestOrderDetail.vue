  ...
  <div class="min-h-screen theme-page pt-24 pb-16">
    <div class="container mx-auto px-4">
      ...
      <div v-else class="space-y-6">
        ...
        <div v-if="order.children && order.children.length > 0"
          ...
          <div class="space-y-4">
            <div v-for="child in order.children" :key="child.id"
              ...
              <div v-if="!codexAccounts.length" class="mt-4">
        ...
        <div v-if="order.fulfillment && !codexAccounts.length"
          class="theme-panel rounded-2xl p-6">
          ...
          <div v-else-if="!fulfillmentFiles.length || (order.fulfillment.payload && order.fulfillment.payload.trim())"
        ...
        <!-- 文件型卡密附件下载区：独立于 order.fulfillment（多子单订单父级可能没有自己的 fulfillment 行） -->
        <div v-if="fulfillmentFiles.length" class="theme-panel rounded-2xl p-6">
          <div class="flex items-center justify-between mb-3 gap-2">
            <div class="text-base font-bold">{{ t('orderDetail.fulfillmentAttachmentsTitle') }}</div>
            <button v-if="fulfillmentFiles.length > 1" type="button"
              class="inline-flex items-center gap-1 text-sm font-medium px-3 py-1.5 rounded-lg bg-emerald-600 text-white hover:bg-emerald-700 active:bg-emerald-800 transition-colors shadow-sm disabled:opacity-50"
              :disabled="fulfillmentArchiveDownloading"
              @click="handleDownloadFulfillmentArchive">
              <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M20 13V6a2 2 0 00-2-2H6a2 2 0 00-2 2v7m16 0v5a2 2 0 01-2 2H6a2 2 0 01-2-2v-5m16 0h-3l-2 3h-6l-2-3H4"/></svg>
              {{ fulfillmentArchiveDownloading ? t('orderDetail.fulfillmentDownloading') : t('orderDetail.fulfillmentAttachmentDownloadAll') }}
            </button>
          </div>
          <div class="space-y-2">
            <div v-for="file in fulfillmentFiles" :key="file.id"
              class="theme-surface-soft border rounded-xl p-3 flex items-center gap-3">
              <svg class="w-5 h-5 shrink-0 text-blue-600" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 10-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13"/></svg>
              <div class="min-w-0 flex-1">
                <div class="text-sm font-medium theme-text-primary truncate" :title="file.filename">{{ file.filename }}</div>
                <div class="text-xs theme-text-muted">{{ formatFulfillmentFileSize(file.size) }}</div>
              </div>
              <button type="button"
                class="shrink-0 inline-flex items-center gap-1 text-sm font-medium px-3 py-1.5 rounded-lg bg-blue-600 text-white hover:bg-blue-700 disabled:opacity-50 transition-colors"
                :disabled="fulfillmentFileDownloading[file.id]"
                @click="handleDownloadFulfillmentFile(file)">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M4 16v2a2 2 0 002 2h12a2 2 0 002-2v-2M7 10l5 5 5-5M12 15V3"/></svg>
                {{ fulfillmentFileDownloading[file.id] ? t('orderDetail.fulfillmentDownloading') : t('orderDetail.fulfillmentAttachmentDownload') }}
              </button>
            </div>
          </div>
        </div>
        ...
        <!-- Codex 号池账号交付 -->
        <div v-if="codexAccounts.length" class="theme-panel rounded-2xl p-6">
          <div class="flex items-center justify-between mb-3 gap-2 flex-wrap">
            <div class="text-base font-bold">{{ t('orderDetail.codexAccountsTitle') }}</div>
            <div v-if="codexAccounts.length > 1" class="flex items-center gap-2">
              <button type="button"
                class="inline-flex items-center gap-1 text-xs font-medium px-3 py-1.5 rounded-lg bg-indigo-600 text-white hover:bg-indigo-700 disabled:opacity-50 transition-colors shadow-sm"
                :disabled="codexArchiveDownloading.cpamc"
                @click="handleDownloadCodexArchive('cpamc')">
                {{ codexArchiveDownloading.cpamc ? t('orderDetail.fulfillmentDownloading') : t('orderDetail.codexDownloadAllCpamc') }}
              </button>
              <button type="button"
                class="inline-flex items-center gap-1 text-xs font-medium px-3 py-1.5 rounded-lg bg-emerald-600 text-white hover:bg-emerald-700 disabled:opacity-50 transition-colors shadow-sm"
                :disabled="codexArchiveDownloading.sub2api"
                @click="handleDownloadCodexArchive('sub2api')">
                {{ codexArchiveDownloading.sub2api ? t('orderDetail.fulfillmentDownloading') : t('orderDetail.codexDownloadAllSub2api') }}
              </button>
            </div>
          </div>
          <p class="text-xs theme-text-muted mb-3">{{ t('orderDetail.codexAccountsHint') }}</p>
          <div class="space-y-2">
            <div v-for="acc in codexAccounts" :key="acc.id"
              class="theme-surface-soft border rounded-xl p-3 flex items-center gap-3 flex-wrap">
              <div class="min-w-0 flex-1">
                <div class="text-sm font-medium theme-text-primary break-all">{{ acc.email || acc.account_id || `#${acc.id}` }}</div>
                <div v-if="acc.plan" class="text-xs theme-text-muted mt-0.5">plan: {{ acc.plan }}</div>
              </div>
              <button type="button"
                class="shrink-0 inline-flex items-center gap-1 text-xs font-medium px-3 py-1.5 rounded-lg bg-slate-600 text-white hover:bg-slate-700 transition-colors"
                @click="handleCopyCodexEmail(acc)">
                {{ codexCopied[acc.id] ? t('orderDetail.fulfillmentCopied') : t('orderDetail.codexCopyEmail') }}
              </button>
              <button type="button"
                class="shrink-0 inline-flex items-center gap-1 text-xs font-medium px-3 py-1.5 rounded-lg bg-indigo-600 text-white hover:bg-indigo-700 disabled:opacity-50 transition-colors"
                :disabled="codexDownloading[`cpamc:${acc.id}`]"
                @click="handleDownloadCodexAccount(acc, 'cpamc')">
                {{ codexDownloading[`cpamc:${acc.id}`] ? t('orderDetail.fulfillmentDownloading') : t('orderDetail.codexDownloadCpamc') }}
              </button>
              <button type="button"
                class="shrink-0 inline-flex items-center gap-1 text-xs font-medium px-3 py-1.5 rounded-lg bg-emerald-600 text-white hover:bg-emerald-700 disabled:opacity-50 transition-colors"
                :disabled="codexDownloading[`sub2api:${acc.id}`]"
                @click="handleDownloadCodexAccount(acc, 'sub2api')">
                {{ codexDownloading[`sub2api:${acc.id}`] ? t('orderDetail.fulfillmentDownloading') : t('orderDetail.codexDownloadSub2api') }}
              </button>
            </div>
          </div>
        </div>
...
const fulfillmentFiles = ref<FulfillmentFile[]>([])
const fulfillmentFileDownloading = ref<Record<string, boolean>>({})

const formatFulfillmentFileSize = (size: number) => {
  if (!size) return ''
...
const loadFulfillmentFiles = async () => {
  const orderNo = order.value?.order_no
  if (!orderNo || !auth.value.email || !auth.value.order_password) {
    fulfillmentFiles.value = []
    return
  }
  try {
    const res = await guestOrderAPI.listFulfillmentFiles(orderNo, {
      email: auth.value.email,
      order_password: auth.value.order_password,
    })
    const list = Array.isArray(res?.data?.data?.files) ? res.data.data.files : []
    fulfillmentFiles.value = list as FulfillmentFile[]
  } catch {
    fulfillmentFiles.value = []
  }
}

const handleDownloadFulfillmentFile = async (file: FulfillmentFile) => {
  const orderNo = file.order_no || order.value?.order_no
  if (!orderNo || !file.id) return
  if (fulfillmentFileDownloading.value[file.id]) return
  fulfillmentFileDownloading.value[file.id] = true
  try {
    const res = await guestOrderAPI.downloadFulfillmentFile(orderNo, file.id, {
      email: auth.value.email,
      order_password: auth.value.order_password,
    })
    ...
    a.href = url
    a.download = file.filename || 'credential.bin'
    a.click()
    URL.revokeObjectURL(url)
  } catch {} finally {
    fulfillmentFileDownloading.value[file.id] = false
  }
}

const fulfillmentArchiveDownloading = ref(false)
const handleDownloadFulfillmentArchive = async () => {
  const orderNo = order.value?.order_no
  if (!orderNo || !auth.value.email || !auth.value.order_password) return
  if (fulfillmentArchiveDownloading.value) return
  fulfillmentArchiveDownloading.value = true
  try {
    const res = await guestOrderAPI.downloadFulfillmentArchive(orderNo, {
      email: auth.value.email,
      order_password: auth.value.order_password,
    })
    ...
    const a = document.createElement('a')
    a.href = url
    a.download = `fulfillment-${orderNo}.zip`
    a.click()
    URL.revokeObjectURL(url)
  } catch {} finally {
    fulfillmentArchiveDownloading.value = false
  }
}
...
const codexAccounts = ref<CodexAccountSummary[]>([])
const codexCopied = ref<Record<number, boolean>>({})
const codexDownloading = ref<Record<string, boolean>>({})
const codexArchiveDownloading = ref<Record<'cpamc' | 'sub2api', boolean>>({ cpamc: false, sub2api: false })
...
const loadCodexAccounts = async () => {
  ...
  try {
    const res = await guestOrderAPI.listCodexAccounts(orderNo, {
      email: auth.value.email,
      order_password: auth.value.order_password,
    })
    const list = Array.isArray(res?.data?.data?.accounts) ? res.data.data.accounts : []
    codexAccounts.value = list as CodexAccountSummary[]
  } catch {
    codexAccounts.value = []
  }
}

const handleCopyCodexEmail = async (acc: CodexAccountSummary) => {
  // 复制 CpaMC 格式 JSON（=原始子订单交付内容）
  const orderNo = order.value?.order_no
  if (!orderNo || !acc?.id || !auth.value.email || !auth.value.order_password) return
  try {
    const res = await guestOrderAPI.downloadCodexAccount(orderNo, 'cpamc', acc.id, {
      email: auth.value.email,
      order_password: auth.value.order_password,
    })
    const text = await blobToText(res.data)
    if (!text) return
    await copyText(text)
    codexCopied.value = { ...codexCopied.value, [acc.id]: true }
    setTimeout(() => {
      codexCopied.value = { ...codexCopied.value, [acc.id]: false }
    }, 1500)
  } catch {}
}

const blobToText = async (data: any): Promise<string> => {
  if (!data) return ''
...
const handleDownloadCodexAccount = async (acc: CodexAccountSummary, format: 'cpamc' | 'sub2api') => {
  const orderNo = order.value?.order_no
  if (!orderNo || !auth.value.email || !auth.value.order_password) return
  const key = `${format}:${acc.id}`
  if (codexDownloading.value[key]) return
  codexDownloading.value = { ...codexDownloading.value, [key]: true }
  try {
    const res = await guestOrderAPI.downloadCodexAccount(orderNo, format, acc.id, {
      email: auth.value.email,
      order_password: auth.value.order_password,
    })
    ...
    const a = document.createElement('a')
    a.href = url
    const safeEmail = (acc.email || `account-${acc.id}`).replace(/[^a-zA-Z0-9@.+\-_]/g, '_')
    a.download = format === 'cpamc' ? `Codex-${safeEmail}.json` : `Sub2api-${safeEmail}.json`
    a.click()
    URL.revokeObjectURL(url)
  } catch {} finally {
    codexDownloading.value = { ...codexDownloading.value, [key]: false }
  }
}

const handleDownloadCodexArchive = async (format: 'cpamc' | 'sub2api') => {
  const orderNo = order.value?.order_no
  if (!orderNo || !auth.value.email || !auth.value.order_password) return
  if (codexArchiveDownloading.value[format]) return
  codexArchiveDownloading.value = { ...codexArchiveDownloading.value, [format]: true }
  try {
    const res = await guestOrderAPI.downloadCodexAccountsArchive(orderNo, format, {
      email: auth.value.email,
      order_password: auth.value.order_password,
    })
    const blob = new Blob([res.data], { type: 'application/zip' })
    ...
    const a = document.createElement('a')
    a.href = url
    a.download = `${format}-codex-${orderNo}.zip`
    a.click()
    URL.revokeObjectURL(url)
  } catch {} finally {
    codexArchiveDownloading.value = { ...codexArchiveDownloading.value, [format]: false }
  }
}
