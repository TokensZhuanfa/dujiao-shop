...
const loadFulfillmentFiles = async () => {
  ...
  try {
    const res = await userOrderAPI.listFulfillmentFiles(orderNo)
...
const handleDownloadFulfillmentFile = async (file: FulfillmentFile) => {
  ...
  try {
    const res = await userOrderAPI.downloadFulfillmentFile(orderNo, file.id)
...
const handleDownloadFulfillmentArchive = async () => {
  ...
  try {
    const res = await userOrderAPI.downloadFulfillmentArchive(orderNo)
...
const codexDownloading = ref<Record<string, boolean>>({}) // key = `${format}:${id}`
...
const loadCodexAccounts = async () => {
  ...
  try {
    const res = await userOrderAPI.listCodexAccounts(orderNo)
...
const handleCopyCodexEmail = async (acc: CodexAccountSummary) => {
  // 名字虽叫 Copy"Email"，实际复制的是 CpaMC 格式 JSON——
  // 即原本子订单交付内容（codex auth.json 单行）。
  const orderNo = order.value?.order_no
  if (!orderNo || !acc?.id) return
  try {
    const res = await userOrderAPI.downloadCodexAccount(orderNo, 'cpamc', acc.id)
...
const handleDownloadCodexAccount = async (acc: CodexAccountSummary, format: 'cpamc' | 'sub2api') => {
  ...
  try {
    const res = await userOrderAPI.downloadCodexAccount(orderNo, format, acc.id)
...
const handleDownloadCodexArchive = async (format: 'cpamc' | 'sub2api') => {
  ...
  try {
    const res = await userOrderAPI.downloadCodexAccountsArchive(orderNo, format)
    const blob = new Blob([res.data], { type: 'application/zip' })
    const url = URL.createObjectURL(blob)
    const a = document.createElement('a')
    a.href = url
