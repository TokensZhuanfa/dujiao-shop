...
const navGroups = computed<NavGroup[]>(() => {
  const groups: NavGroup[] = [
    ...
    {
      id: 'number-pool',
      label: t('admin.navGroups.numberPool'),
      icon: Layers,
      items: [
        {
          label: t('admin.navItems.numberPoolCodex'),
          to: '/number-pool/codex',
          icon: Bot,
          permission: 'GET:/admin/number-pool/codex',
        },
      ],
    },
    {
      ...
      >
        ...
        <div class="border-t border-border">
          <div v-if="!sidebarCollapsed" class="px-6 py-3 text-[11px] text-muted-foreground space-y-1">
            <p>© {{ new Date().getFullYear() }} Dujiao-Shop <span v-if="appVersion" class="text-muted-foreground/70">{{ appVersion }}</span></p>
            <a
              href="https://github.com/TokensZhuanfa/dujiao-shop"
              target="_blank"
            ...
            >
              ...
              <span>https://github.com/TokensZhuanfa/dujiao-shop</span>
            </a>
          </div>
          <button
            type="button"
