...
// --- Shared file (file_shared mode) management ---
const sharedFileSubmitting = ref(false)
const sharedFileError = ref('')
const sharedFileSuccess = ref('')

const productAutoSecretKind = computed(() => productInfo.value?.auto_secret_kind || 'text')
const productSharedFileName = computed(() => productInfo.value?.shared_file_original_name || '')
const productSharedFileSize = computed(() => productInfo.value?.shared_file_size || 0)

const formatSharedSize = (size: number) => {
  if (!size) return ''
...
const handleSharedFileChange = async (files: FileList | null) => {
  if (!currentProductId.value || !files || !files.length) return
  const file = files[0]
  if (!file) return
  sharedFileError.value = ''
  sharedFileSuccess.value = ''
  sharedFileSubmitting.value = true
  try {
    const fd = new FormData()
    fd.append('file', file, file.name)
    await adminAPI.uploadProductSharedFile(currentProductId.value, fd)
    sharedFileSuccess.value = t('admin.cardSecretImports.sharedFile.uploaded')
    await loadProductInfo()
  } catch (err: any) {
    sharedFileError.value = err.message || t('admin.cardSecrets.errors.importFailed')
  } finally {
    sharedFileSubmitting.value = false
  }
}

const handleSharedFileDelete = async () => {
  if (!currentProductId.value) return
  sharedFileError.value = ''
  sharedFileSuccess.value = ''
  sharedFileSubmitting.value = true
  try {
    await adminAPI.deleteProductSharedFile(currentProductId.value)
    sharedFileSuccess.value = t('admin.cardSecretImports.sharedFile.deleted')
    await loadProductInfo()
  } catch (err: any) {
    sharedFileError.value = err.message || t('admin.cardSecrets.errors.batchFailed')
  } finally {
    sharedFileSubmitting.value = false
  }
}
...
<template>
  <div class="space-y-6">
    ...
    <div v-else class="space-y-4">
      ...
      <!-- Shared file (file_shared mode) -->
      <div class="rounded-xl border border-border bg-card p-6">
        <div class="flex items-center justify-between gap-4 mb-3">
          <h2 class="text-lg font-semibold text-foreground">{{ t('admin.cardSecretImports.sharedFile.title') }}</h2>
          <span class="text-xs text-muted-foreground">{{ t('admin.cardSecretImports.sharedFile.modeLabel') }}: <span class="font-medium text-foreground">{{ productAutoSecretKind }}</span></span>
        </div>
        <p class="text-xs text-muted-foreground mb-3">{{ t('admin.cardSecretImports.sharedFile.hint') }}</p>
        <div v-if="productSharedFileName" class="rounded-lg border border-emerald-200 bg-emerald-50 p-3 text-sm text-emerald-700 mb-3 flex items-center justify-between gap-2">
          <span class="truncate" :title="productSharedFileName">{{ productSharedFileName }}</span>
          <span class="shrink-0 text-xs">{{ formatSharedSize(productSharedFileSize) }}</span>
        </div>
        <div class="flex flex-wrap items-center gap-2">
          <FileInput
            :button-text="productSharedFileName ? t('admin.cardSecretImports.sharedFile.replace') : t('admin.cardSecretImports.sharedFile.upload')"
            :disabled="sharedFileSubmitting"
            @change="handleSharedFileChange"
          />
          <Button
            v-if="productAutoSecretKind === 'file_shared'"
            type="button"
            size="sm"
            variant="ghost"
            :disabled="sharedFileSubmitting"
            @click="handleSharedFileDelete"
          >{{ t('admin.cardSecretImports.sharedFile.delete') }}</Button>
        </div>
        <div v-if="sharedFileError" class="mt-3 rounded-lg border border-destructive/30 bg-destructive/10 p-3 text-sm text-destructive">
          {{ sharedFileError }}
        </div>
        <div v-if="sharedFileSuccess" class="mt-3 rounded-lg border border-emerald-200 bg-emerald-50 p-3 text-sm text-emerald-700">
          {{ sharedFileSuccess }}
        </div>
      </div>
    </div>
