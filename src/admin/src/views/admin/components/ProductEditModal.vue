...
const populateForm = (product: AdminProduct) => {
  ...
  Object.assign(form, {
    ...
    auto_secret_kind: product.auto_secret_kind || 'text',
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const payload = {
      ...
      auto_secret_kind: form.fulfillment_type === 'auto' ? (form.auto_secret_kind || 'text') : 'text',
      ...
      <form class="space-y-6" @submit.prevent="handleSubmit">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div v-if="form.fulfillment_type === 'auto' && !editingIsMapped" class="col-span-1">
            <label class="block text-xs font-medium text-muted-foreground mb-1.5">{{ t('admin.products.form.autoSecretKind') }}</label>
            <Select v-model="form.auto_secret_kind">
              <SelectTrigger class="h-9 w-full">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="text">{{ t('admin.products.autoSecretKind.text') }}</SelectItem>
                <SelectItem value="file_per_stock">{{ t('admin.products.autoSecretKind.filePerStock') }}</SelectItem>
                <SelectItem value="file_shared">{{ t('admin.products.autoSecretKind.fileShared') }}</SelectItem>
                <SelectItem value="codex_pool">{{ t('admin.products.autoSecretKind.codexPool') }}</SelectItem>
              </SelectContent>
            </Select>
            <p class="mt-1 text-xs text-muted-foreground">{{ t('admin.products.autoSecretKindTip.' + (form.auto_secret_kind || 'text')) }}</p>
          </div>
