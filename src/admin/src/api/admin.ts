...
export const adminAPI = {
  ...
  importCardSecretFiles: (formData: FormData) =>
    api.post('/admin/card-secrets/import-files', formData),
  uploadProductSharedFile: (productId: number, formData: FormData) =>
    api.post(`/admin/products/${productId}/shared-file`, formData),
  deleteProductSharedFile: (productId: number) =>
    api.delete(`/admin/products/${productId}/shared-file`),

  // 号池 - Codex
  listCodexAccounts: (params?: { page?: number; page_size?: number; keyword?: string; status?: string; plan?: string }) =>
    api.get('/admin/number-pool/codex', { params }),
  getCodexPoolSettings: () => api.get('/admin/number-pool/codex/settings'),
  saveCodexPoolSettings: (data: {
    ...
    proxy_urls: string
    proxy_url?: string
  }) => api.put('/admin/number-pool/codex/settings', data),
  testCodexProxy: (proxy_url: string) =>
    api.post('/admin/number-pool/codex/proxy/test', { proxy_url }),
  importCodexAccounts: (data: { json: string; alias_prefix?: string; tags?: string[]; note?: string }) =>
    api.post('/admin/number-pool/codex/import', data),
  importCodexAccountsFiles: (formData: FormData) =>
    api.post('/admin/number-pool/codex/import', formData),
  updateCodexAccount: (id: number, data: { alias?: string; tags?: string[]; note?: string; sold?: boolean }) =>
    api.patch(`/admin/number-pool/codex/${id}`, data),
  setCodexAccountStatus: (id: number, status: string) =>
    api.patch(`/admin/number-pool/codex/${id}/status`, { status }),
  refreshCodexAccount: (id: number) =>
    api.post(`/admin/number-pool/codex/${id}/refresh`),
  fetchCodexAccountUsage: (id: number) =>
    api.post(`/admin/number-pool/codex/${id}/usage`),
  batchCodexAccount: (action: 'delete' | 'refresh' | 'usage', ids: number[]) =>
    api.post('/admin/number-pool/codex/batch', { action, ids }),
  deleteCodexAccount: (id: number) =>
    api.delete(`/admin/number-pool/codex/${id}`),
